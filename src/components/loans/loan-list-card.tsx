...
export function LoanListCard({ loan, role = "lender" }: { loan: LoanListItem; role?: "lender" | "borrower" }) {
  ...
  return (
    <Link
      href={`/loans/${loan.id}`}
      className={`group/card flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 sm:p-5 border-[2px] border-black dark:border-white/40 bg-[#FAF8F5] dark:bg-[var(--card)] text-black dark:text-white shadow-[3px_3px_0_0_#000] hover:-translate-y-0.5 active:translate-y-0.5 active:shadow-[1px_1px_0_0_#000] transition-all font-mono ${
        isLender
    ...
    >
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <p className="text-base sm:text-lg font-black tracking-tight text-black dark:text-white group-hover/card:text-black dark:group-hover/card:text-black transition-colors">
            {loan.counterparty.full_name ?? `@${loan.counterparty.username}`}
          </p>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 group-hover/card:text-black dark:group-hover/card:text-black font-bold mt-0.5 flex items-center gap-1.5 flex-wrap transition-colors">
            <span className="group-hover/card:text-black dark:group-hover/card:text-black">{interestSummary(loan)}</span>
            <span className="group-hover/card:text-black dark:group-hover/card:text-black">·</span>
            <span className="flex items-center gap-1 group-hover/card:text-black dark:group-hover/card:text-black">
      ...
      <div className="flex items-center justify-between sm:justify-end gap-4 border-t sm:border-t-0 pt-2 sm:pt-0 border-black/10 dark:border-white/10">
        <div className="text-left sm:text-right">
          <span className="text-[10px] uppercase text-gray-500 dark:text-gray-400 group-hover/card:text-black dark:group-hover/card:text-black block font-bold transition-colors">
            {isLender ? "To Collect" : "To Repay"}
          </span>
          <p className={`font-mono text-lg sm:text-xl font-black group-hover/card:text-black dark:group-hover/card:text-black transition-colors ${
